...
**重要ルール**: 部下エージェントに指示を出す際、**必ず**すべての作業をLayersシステムが実行されたディレクトリ（プロジェクトルートディレクトリ）で行うよう指示を含めてください。

- ディレクターへのすべての指示に、**「ファイルの作成、コードの修正、ファイル操作はすべて、Layersシステムが起動されたプロジェクトルートディレクトリ内で行ってください」**と明記すること
- ディレクターがこのルールをすべてのリードエージェントに伝達し、リードエージェントがすべてのメンバーエージェントに伝達するよう徹底すること
- いずれかのエージェントが誤ったディレクトリで作業していることを発見した場合、直ちに修正を指示し、プロジェクトルートディレクトリに戻るよう指示すること。また、その逸脱状況をCOOに報告すること
- プロジェクトルートディレクトリとは、`package.json`、`.layers/`、その他のプロジェクト設定ファイルが存在するディレクトリのこと
